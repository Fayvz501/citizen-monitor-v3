...
router.post('/incidents/:uid/comments', authMiddleware, (req,res)=>{try{const{text}=req.body;if(!text?.trim())return res.status(400).json({error:'Empty'});const mod=moderateText(text);if(!mod.passed)return res.status(400).json({error:'Moderation'});const db=getDb();const inc=db.prepare('SELECT id,user_id FROM incidents WHERE uid=?').get(req.params.uid);if(!inc)return res.status(404).json({error:'Not found'});const r=db.prepare('INSERT INTO comments (incident_id,user_id,text) VALUES (?,?,?)').run(inc.id,req.user.id,text.trim());const c=db.prepare('SELECT c.*,u.username,u.avatar_color,u.reputation,u.trust_level,u.vk_photo FROM comments c JOIN users u ON c.user_id=u.id WHERE c.id=?').get(r.lastInsertRowid);if(inc.user_id!==req.user.id)db.prepare("INSERT INTO notifications (user_id,incident_id,type,message) VALUES (?,?,?,?)").run(inc.user_id,inc.id,'comment',`${req.user.username} прокомментировал`);checkAndAward(req.user.id);notifyVk('/notify/comment',{incident_uid:req.params.uid,username:req.user.username,text:text.trim()});res.json({comment:c});}catch(e){res.status(500).json({error:'Error'});}});
